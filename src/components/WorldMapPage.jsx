...
const WorldMapPage = () => {
  ...
  const listaDePaisesComOportunidades = useMemo(() => agruparOportunidadesPorPais(oportunidades), []);
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-950 text-white font-inter">
      
      
      <div className={`w-full md:basis-3/4 md:h-full flex items-center justify-center relative ${baseTransition} ${showMap ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'} ${clickedCountryData && isMobileView ? 'h-0 opacity-0 z-10' : 'flex-1 md:h-full opacity-100 z-10'}`}>
        <WorldMap
      ...
      <div className={`w-full md:basis-1/4 md:h-full flex flex-col bg-slate-900 shadow-lg relative border border-slate-950 ${baseTransition} ${showSidebar ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'} ${clickedCountryData && isMobileView ? 'h-full z-20' : 'flex-1 md:h-full z-20'}`}>
        {clickedCountryData && (
          <button
            onClick={handleCloseSidebar}
            className="absolute top-4 right-4 text-white hover:text-amber-500 transition-colors z-30"
        ...
        <div className="px-6 py-4">
          <h2 className="text-xl font-bold mb-4 border-b pb-2 border-slate-950 text-amber-500">
        ...
        <div id="map-sidebar-scroll" className="flex-grow overflow-y-auto px-6 py-4 pt-0">
          {!clickedCountryData ? (
            <div>
              ...
              <h3 className="text-lg font-semibold mt-4 mb-2 hidden md:block text-white">Países com Oportunidades</h3>
              <div className="grid grid-cols-2 gap-2 text-sm hidden md:grid">
