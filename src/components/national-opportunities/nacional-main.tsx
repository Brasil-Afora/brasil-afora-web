...
const applyOpportunityFilters = (
  ...
  if (filtros.modalidade.length > 0) {
    result = result.filter((op) => filtros.modalidade.includes(op.modalidade))
  }
  return result
}
...
const NacionalMain = () => {
  ...
  }, [filtros, oportunidadesNacionais])
  ...
  const renderAppliedFilters = () => {
    ...
    for (const m of filtros.modalidade) {
      applied.push({ key: "modalidade", value: m })
    }
          ...
          {!isFilterActive() && (
            ...
            >
              ...
              <p className="mx-auto mt-2 max-w-xl text-sm text-white/50">
                Encontre olimpíadas, feiras científicas e projetos de liderança
                no Brasil.
              </p>
            </div>
          )}
