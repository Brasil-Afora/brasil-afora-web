...
const applyOpportunityFilters = (
  ...
  if (filtros.tipoBolsa.length > 0) result = result.filter((op) => filtros.tipoBolsa.includes(op.tipoBolsa))
  return result
}
...
const InternacionalMain = () => {
  ...
  const renderAppliedFilters = () => {
    ...
    for (const p of filtros.pais) applied.push({ key: "pais", value: p })
    ...
    for (const i of filtros.requisitosIdioma) applied.push({ key: "requisitosIdioma", value: i })
    for (const t of filtros.taxaAplicacao) applied.push({ key: "taxaAplicacao", value: t })
    for (const t of filtros.tipoBolsa) applied.push({ key: "tipoBolsa", value: t })
      ...
      {!isFilterActive() && (
        ...
        >
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm text-white/50">
            Explore bolsas de estudo, summer camps e intercâmbios ao redor do
            mundo.
          </p>
        </div>
      )}
