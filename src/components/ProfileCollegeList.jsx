...
import { FaTrash, FaChevronDown, FaChevronUp, FaSort, FaSortAmountUp, FaSortAmountDown, FaExternalLinkAlt, FaPlus } from 'react-icons/fa';
    ...
    return (
        <div>
            ...
            <div className="flex items-center flex-wrap gap-2 mb-6">
                <span className="font-semibold text-lg mr-2 text-white">Ordenar por:</span>
                <button
                    onClick={() => handleSort('taxaAceitacao')}
                    className={`px-4 py-2 rounded-full font-semibold flex items-center gap-2 transition-colors duration-200 ${sortCriteria === 'taxaAceitacao' ? 'bg-amber-500 text-black' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                >
                    Taxa de Aceitação
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-full font-semibold flex items-center gap-2 transition-colors duration-200 ${sortCriteria === 'anuidade' ? 'bg-amber-500 text-black' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                >
                    Anuidade
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-full font-semibold flex items-center gap-2 transition-colors duration-200 ${sortCriteria === 'indiceAjuda' ? 'bg-amber-500 text-black' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                >
                    Índice de Ajuda
            ...
            <div className="flex flex-col gap-4">
                {sortedCollegeList.length > 0 ? (
                    sortedCollegeList.map(uni => {
                        ...
                        return (
                            <div key={uni.id} className="bg-slate-900 p-6 rounded-lg shadow-xl flex flex-col gap-4 border border-slate-950">
                                <div className="flex justify-between items-center">
                                    <div className="cursor-pointer flex-1" onClick={() => handleToggleExpand(uni.id)}>
                                        <div className="flex items-center gap-2 text-white">
                                            <h3 className="text-2xl font-bold">{uni.nome}</h3>
                                            <span className="text-lg font-medium text-amber-500">({progressText})</span>
                                        </div>
                                    ...
                                    <div className="flex items-center space-x-4 ml-auto">
                                        <a
                                            ...
                                            className="bg-slate-950 text-amber-500 font-bold py-2 px-4 rounded-full text-center text-sm hover:scale-105 transition-transform duration-200 flex items-center hover:bg-slate-800"
                                            title="Visitar site"
                                        >
                                        ...
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleToggleExpand(uni.id); }} 
                                            className="bg-slate-950 hover:bg-slate-800 p-2 rounded-full text-white"
                                        >
                                            {expandedUniId === uni.id ? <FaChevronUp /> : <FaChevronDown />}
                                        </button>
                                    </div>
                                </div>
                                
                                {expandedUniId === uni.id && (
                                    <div className="mt-4 border-t border-slate-950 pt-4 grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div>
                                            <h4 className="text-xl font-bold mb-2 text-amber-500">Detalhes da Universidade</h4>
                                            <div className="flex flex-col gap-2 text-slate-400">
                                                <p className="text-sm"><span className="font-semibold text-white">Tipo de Campus:</span> <span>{uni.setting}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Taxa de Aceitação:</span> <span>{uni.taxaAceitacao}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Anuidade:</span> <span>{uni.tuition}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Faixa de SAT:</span> <span>{uni.faixaSAT}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Faixa de ACT:</span> <span>{uni.faixaACT}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Cursos Populares:</span> <span>{uni.majorsPrincipais}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Taxa de Inscrição:</span> <span>{uni.applicationFee}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Pacote de Auxílio Médio:</span> <span>{uni.averageNeedBasedAidPackage}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Total de Alunos:</span> <span>{uni.totalAlunos}</span></p>
                                                <p className="text-sm"><span className="font-semibold text-white">Plataforma de Inscrição:</span> <span>{uni.plataformaInscricao}</span></p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 md:mt-0">
                                            ...
                                            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-2">
                                                {checklistItems.map((item, itemIndex) => (
                                                    ...
                                                    >
                                                        <div className="flex items-start space-x-3 flex-1 cursor-pointer" onClick={() => handleChecklistItem(uni.id, itemIndex)}>
                                                            ...
                                                            />
                                                            <span className={`text-base break-words ${item.completed ? 'line-through text-white' : 'text-white'}`}>
                                                                {item.text}
                                                            </span>
                                                        </div>
                                                        <button 
